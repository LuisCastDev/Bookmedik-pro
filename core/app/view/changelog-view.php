<div class="row">
<div class="col-md-12">
<h1>Sistema de Citas Medicas</h1>
<p>Bienvenido a <b>COntrol Pacientes</b> un Sistema de Citas Medicas util para consultorios medicos y/o medicos independientes.</p>
<h4> v2.0</h4>
<ul>
	<li>Nuevos campos en pacientes: genero, fecha de nacimiento, Síntomas e Historia de la Enfermedad, medicamento, Examen físico</li>
	<li>Nuevos campos en medicos: genero, fecha de nacimiento </li>
	<li>Nuevos campos al agregar citas: Síntomas e Historia de la Enfermedad, sintomas, Antecedentes del paciente, estado, tipo de pago y precio.</li>
	<li>Reportes por medico, paciente, rango de fecha, estado y tipo de pago +descarga en formato word</li>
</ul>
<h4> v1.5</h4>
<ul>
	<li>+ Vista del Calendario</li>
	<li>Asociar medicos con Areas Medicas. </li>
	<li>Correcion del Bug al agregar Areas medicas</li>
</ul>
<h4> v1.0</h4>
<ul>
	<li>Gestion de Citas</li>
	<li>Gestion de Medicos</li>
	<li>Gestion de Pacientes</li>
	<li>Gestion de Usuarios con Acceso al Sistema</li>
	<li>Historial de Citas por Paciente</li>
	<li>Historial de Citas por Medico</li>
	<li>Buscador avanzado por : Palabra clave, medico, paciente y fecha.</li>
</ul>
<p>Support IT SRL &copy; 2022</p>
</div>
</div>
